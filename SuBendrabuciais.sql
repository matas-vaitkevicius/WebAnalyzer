...
isbendrabutis l_r_isBendrabutis
  FROM [WebAnalyzer].[dbo].Rent
  where url like '%aruodas%'
  and HeatingType not like '%Kietu kuru%'
group by SUBSTRING(Title,1,4) ,
SUBSTRING(Title, CHARINDEX(',', Title, 0)+2, CASE WHEN CHARINDEX(',', Title, CHARINDEX(',', Title, 0)+1 ) - CHARINDEX(',', Title, 0) - 2 < 0 then 0 else CHARINDEX(',', Title, CHARINDEX(',', Title, 0)+1 ) - CHARINDEX(',', Title, 0) - 2 end) 
, RoomCount, isBendrabutis) r on r.l_r_roomC = s.l_roomC and r.l_r_miestas = s.miestas and r.l_r_rajonas = s.rajonas and r.l_r_isBendrabutis = s.l_isBendrabutis) by_dist
join (select  * from 
  ...
  , isbendrabutis r_isbendrabutis
  FROM [WebAnalyzer].[dbo].Rent 
  where url like '%aruodas%'   and HeatingType not like '%Kietu kuru%'
  group by  SUBSTRING(Title,1,4), coalesce(RoomCount,-1), isBendrabutis ) r on r.r_name = s.name and r.r_roomcount = s.roomC and r.r_isbendrabutis = s.isbendrabutis) by_town) bt on bt.name = by_dist.miestas and bt.roomC = by_dist.l_roomC and bt.isbendrabutis = by_dist.l_isBendrabutis ) analysis
  inner join Sale ads on analysis.miestas = SUBSTRING(Title,1,4) and analysis.rajonas = SUBSTRING(Title, CHARINDEX(',', Title, 0)+2, CASE WHEN CHARINDEX(',', Title, CHARINDEX(',', Title, 0)+1 ) - CHARINDEX(',', Title, 0) - 2 < 0 then 0 else CHARINDEX(',', Title, CHARINDEX(',', Title, 0)+1 ) - CHARINDEX(',', Title, 0) - 2 end)  and analysis.roomC  = coalesce(ads.RoomCount,-1) and analysis.l_isBendrabutis = ads.isBendrabutis
  where 
 ((
...
and analysis.l_cnt > 2 and analysis.l_r_cnt > 2 and analysis.l_roomC > 0 
   and HeatingType not like '%Kietu kuru%'
